...
let createNewOrder = async (data) => {
    try {
        ...
        let orderDetails = data.arrDataShopCart.map((item) => ({
            ...item,
        ...
        if (cart) {
            ...
            for (let item of data.arrDataShopCart) {
                ...
                if (!productDetailSize) {
                    console.warn(`Không tìm thấy ProductDetailSize với id: ${item.productId}`)
                    continue
                }

                if (productDetailSize.stock < item.quantity) {
                    return {
                        errCode: 2,
                        errMessage: `Sản phẩm id ${item.productId} không đủ tồn kho`
                    }
                }
...
let getDetailOrderById = async (id) => {
    try {
        if (!id) {
        ...
        let addressUser = await db.AddressUser.findOne({ where: { id: order.addressUserId } })
...
let getAllOrdersByUser = async (userId) => {
    try {
        if (!userId) {
            return { errCode: 1, errMessage: 'Missing required parameter !' }
        }

        let addressUser = await db.AddressUser.findAll({ where: { userId: userId } })
        ...
        return { errCode: 0, data: addressUser }
...
let getAllOrdersByShipper = async (data) => {
    try {
    ...
    try {
        ...
        var create_payment_json = {
            ...
            "transactions": [{
                ...
                "amount": { "currency": "USD", "total": data.total },
                "description": "This is the payment description."
            }]
        };

        // paypal.payment.create dùng callback → wrap lại bằng Promise
        return await new Promise((resolve) => {
            paypal.payment.create(create_payment_json, function (error, payment) {
                if (error) {
                    resolve({ errCode: -1, errMessage: error })
                } else {
                    resolve({ errCode: 0, errMessage: 'ok', link: payment.links[1].href })
                }
            })
        })
...
let paymentOrderSuccess = async (data) => {
    try {
        ...
        // paypal.payment.execute dùng callback → wrap lại bằng Promise
        return await new Promise((resolve) => {
            paypal.payment.execute(data.paymentId, execute_payment_json, async function (error, payment) {
                if (error) {
                    resolve({ errCode: 0, errMessage: error })
                } else {
                    try {
                        let product = await db.OrderProduct.create({
                            addressUserId: data.addressUserId,
...
let paymentOrderVnpaySuccess = async (data) => {
    try {
        let product = await db.OrderProduct.create({
            addressUserId: data.addressUserId,
            isPaymentOnlien: data.isPaymentOnlien,
...
let paymentOrderVnpay = async (req) => {
    try {
        ...
        return { errCode: 200, link: vnpUrl }
...
let confirmOrderVnpay = async (data) => {
    try {
        var vnp_Params = data;
        ...
        if (secureHash === signed) {
            return { errCode: 0, errMessage: 'Success' }
        } else {
            return { errCode: 1, errMessage: 'failed' }
        }
...
let updateImageOrder = async (data) => {
    try {
...
module.exports = {
    createNewOrder, getAllOrders, getDetailOrderById,
    updateStatusOrder, getAllOrdersByUser, getAllOrdersByShipper,
    paymentOrder, paymentOrderSuccess, confirmOrder,
    paymentOrderVnpay, confirmOrderVnpay, paymentOrderVnpaySuccess,
    updateImageOrder
}